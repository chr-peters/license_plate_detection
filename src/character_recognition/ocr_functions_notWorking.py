...
def extract_plate_side(img, bounding_box, change_x, change_y, coords):
    ...
    if coords == 1: # topright
        img_bound = img[y - change_y : y + h - change_y, x + change_x : x + w + change_x]
        x_start = x + change_x
        y_start = y - change_y
...
def ocr_extraction(sorted_contours, gray, thresh, x_start, y_start):
    ...
    if not data_plate.empty: 
        ...
        data_plate = data_plate.drop(columns=['level', 'page_num', 'block_num', 
...
def ocr(img, bounding_box, method):
    ...
    if method == "normal":
        ...
        data_plate, plate_num = ocr_extraction(sorted_contours, gray, thresh, 
                                               x_start, y_start)
        
    if method == "up":
        img_bound_up, x_startup, y_startup = extract_plate_ver(img, bounding_box, 
    ...
    if method == "down":
        img_bound_down, x_startdown, y_startdown = extract_plate_ver(img, bounding_box, 
                                                    int(bounding_box[1]*img.shape[0]*0.1),
                                                    False)
        sorted_contours, gray, thresh = image_preproc(img_bound)
        data_plate, plate_num = ocr_extraction(sorted_contours, gray, thresh,
    ...
    if method == "left":
        img_bound_left, x_startleft, y_startleft = extract_plate_hor(img, bounding_box, 
    ...
    if method == "right":
        img_bound_right, x_startright, y_startright = extract_plate_hor(img, bounding_box, 
                                                    int(bounding_box[0]*img.shape[1]*0.1), 
                                                    False)
        sorted_contours, gray, thresh = image_preproc(img_bound)
        data_plate, plate_num = ocr_extraction(sorted_contours, gray, thresh,
    ...
    if method == "topright":    
        img_bound_tr, x_starttr, y_starttr = extract_plate_side(img, bounding_box, 
                                       int(bounding_box[0]*img.shape[1]*0.1), 
                                       int(bounding_box[1]*img.shape[0]*0.1), 1)
        sorted_contours, gray, thresh = image_preproc(img_bound)
        data_plate, plate_num = ocr_extraction(sorted_contours, gray, thresh,
                                               x_start, y_start)
    
    if method == "bottomright":    
        img_bound_br, x_startbr, y_startbr = extract_plate_side(img, bounding_box, 
                                       int(bounding_box[0]*img.shape[1]*0.1), 
                                       int(bounding_box[1]*img.shape[0]*0.1),2)
        sorted_contours, gray, thresh = image_preproc(img_bound)
        data_plate, plate_num = ocr_extraction(sorted_contours, gray, thresh,
                                               x_start, y_start)

    if method == "bottomleft":
        img_bound_bl, x_startbl, y_startbl = extract_plate_side(img, bounding_box, 
                                       int(bounding_box[0]*img.shape[1]*0.1), 
                                       int(bounding_box[1]*img.shape[0]*0.1), 3)
        sorted_contours, gray, thresh = image_preproc(img_bound)
        data_plate, plate_num = ocr_extraction(sorted_contours, gray, thresh,
                                               x_start, y_start)
    
    if method == "topleft":
        img_bound_tl, x_starttl, y_starttl = extract_plate_side(img, bounding_box, 
                                       int(bounding_box[0]*img.shape[1]*0.1), 
                                       int(bounding_box[1]*img.shape[0]*0.1), 4)
        sorted_contours, gray, thresh = image_preproc(img_bound)
        data_plate, plate_num = ocr_extraction(sorted_contours, gray, thresh, 
                                               x_start, y_start)
...
def ocr_validation(data_plate, plate_num, methods):
    ...
    for i in indices: 
        ...
        for m in methods: 
            ...
            if cur_frame.empty: continue
            cur_opp = cur_frame[(cur_frame.width > width - 2) & (cur_frame.width < width + 2)]
            cur_opp = cur_frame[(cur_frame.height > height - 2) & (cur_frame.height < height + 2)]
            if cur_opp.empty: continue
            indi = cur_opp.index.tolist()
            if len(cur_opp.index) == 1:
                if cur_opp.text[indi].to_string()[len(cur_opp.text[indi].to_string()) - 1] == char:
                    continue
                else: 
                    if (cur_opp.conf > conf).bool():
                        candidate = candidate.replace(char, cur_opp.text[indi].to_string()[len(cur_opp.text[indi].to_string()) - 1])
                    else: 
                        continue
            else:
                for j in indi:
                    if cur_opp.conf[j] > conf:
                        candidate = candidate.replace(char, cur_opp.text[j].to_string()[len(cur_opp.text[j].to_string()) - 1])
                    else: continue                       
